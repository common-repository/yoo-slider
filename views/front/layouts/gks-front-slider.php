<?php
    ...
    if ($isCarousel) {
        ...
        $textButtonDisplyStyle = !empty($gks_slider->options[GKSOption::kTextButtonDisplayStyle]) ? $gks_slider->options[GKSOption::kTextButtonDisplayStyle] : GKSTextButtonDisplayStyle::Bordered;
    ...
    $isTemplateShowcase = false;
    if (isset($_GET['template']) && (int)$_GET['template']) {
      $isTemplateShowcase = true;
    }

?>

<?php if($isTemplateShowcase): ?>
  <div class="gks-template-front-title-block">
    <h1><?php echo wp_kses($gks_slider->title, GKS_ALLOWED_HTML); ?></h1>
  </div>
<?php endif; ?>
...
<?php
  if (GKS_PKG_TYPE == GKS_PKG_TYPE_PREMIUM) {
    if ($isScrollerAvailable && !$isCarousel && $scrollerPosition == GKSScrollerPosition::before) {
      echo gks_render_scroller_html($gks_slider); //Already escaped html
    }
  }
?>
...
<div id="gks-slider-<?php echo esc_attr($gks_slider->id); ?>"  data-gks-id="<?php echo esc_attr($gks_slider->id); ?>" class="gks-slider-layout <?php echo ($isCarousel ? 'gks-slider-carousel-layout' : ''); ?>">
    <?php if ($gks_slider->options[GKSOption::kShowArrows]) { ?>
    <a class="gks-slider-ctrl gks-slider-ctrl-prev"><i class="fa gks-fa <?php echo esc_attr($leftArrClass); ?>"></i></a>
    <a class="gks-slider-ctrl gks-slider-ctrl-next"><i class="fa gks-fa <?php echo esc_attr($rightArrClass); ?>"></i></a>
    <?php } ?>
    <div class="owl-carousel <?php echo $isCarousel ? "gks-carousel" : "" ?> <?php echo $isSlideAnimationsAvailable ? esc_attr($gks_slider->options[GKSOption::kSlideAnimation]) : ''?> <?php echo $isStagingAvailable ? 'stage-style' : ''?>">
        <?php
            foreach ($gks_slider->slides as $gks_slide) {
                ...
                $cssselector = isset($gks_slide->csssel) ? $gks_slide->csssel : "";
                ...
                if ($coverType != GKSAttachmentType::HTML) {
                  ...
                  if (GKS_PKG_TYPE == GKS_PKG_TYPE_PREMIUM) {
                    ...
                    if ($coverType == GKSAttachmentType::VIDEO) {
                        ...
                        $lgJSData .= "
                          { facebookShareUrl: '" . esc_url($metaOriginal['src']) . "',
                            twitterShareUrl: '" . esc_url($metaOriginal['src']) . "',
                            pinterestShareUrl: '" . esc_url($metaOriginal['src']) . "',
                            googleplusShareUrl: '" . esc_url($metaOriginal['src']) . "',
                            tweetText: '" . esc_html($shareText) . "',
                            pinterestText: '" . esc_html($shareText) . "',
                            id: '" . esc_attr($gks_slide->id) . "',
                            html: '#gks-html5video-" . esc_attr($gks_slide->id) . "-" . esc_attr($coverInfo->video->id) . "',
                            thumb: '" . esc_url($metaOriginal["src"]) . "',
                            subHtml: '" . wp_kses($subHtml, GKS_ALLOWED_HTML) . "',
                            downloadUrl: " . ($url ? "'". esc_url($url) . "'" : "false") . "
                          }";
                    } else {
                        ...
                        $lgJSData .= "
                          { facebookShareUrl: '" . esc_url($metaOriginal['src']) . "',
                            ...
                            pinterestText: '" . esc_html($shareText) . "',
                            id: '" . esc_attr($gks_slide->id) . "',
                            src: '" . esc_url($metaOriginal["src"]) . "',
                            thumb: '" . esc_url($meta["src"]) . "',
                            subHtml: '" . wp_kses($subHtml, GKS_ALLOWED_HTML) . "', 'downloadUrl': " . ($url ? "'". esc_url($url) ."'" : "false") . "
                          }";
                    }
                ...
                if (!$isCarousel) {
                    ?>
                    <div id="<?php echo esc_attr($slideId++); ?>" class="gks-slider-cell <?php echo esc_attr($pictureHoverEffect); ?> <?php echo esc_attr($cssselector); ?>" data-type="<?php echo esc_attr($coverType); ?>" data-index="<?php echo esc_attr($coverType != GKSAttachmentType::HTML ? $slideIndex++ : ''); ?>" data-src="<?php echo esc_url(isset($metaOriginal['embed_src']) ? $metaOriginal['embed_src'] : ''); ?>">
                            ...
                            <?php
                              if($coverType != GKSAttachmentType::HTML) {
                                if ($gks_slider->options[GKSOption::kSliderAutoHeight]) {
                                    $imgHtml = '<img class="gks-slider-auto-image gks-tile-img" src="' . esc_url($meta['src']) . '" alt="' . wp_kses($title, GKS_ALLOWED_HTML) . '">';
                                } else {
                                    $imgHtml = '<div class="gks-slider-image gks-tile-img" style="background-image: url(' . esc_url($meta['src']) . '"></div>';
                                }
                                echo $coverType == GKSAttachmentType::PICTURE && !$isShowPopup ? '<a href="' . esc_url($url) . '" ' . ($blank ? 'target="_blank"' : '') . '>' . $imgHtml . '</a>' : $imgHtml;

                                if (!empty($playIconHtml)) {
                                  echo wp_kses($playIconHtml, GKS_ALLOWED_HTML);
                                }
                              } else {
                                echo wp_kses($coverHtml, GKS_ALLOWED_HTML);
                              }
                            ?>
                        </div>
                        <?php if (!empty($title) || !empty($details)) {
                            ...
                            if ($gks_slider->options[GKSOption::kShowTitle] && !empty($title)) {
                                echo '<div class="gks-slider-title">' . wp_kses($title, GKS_ALLOWED_HTML) . '</div>';
                            }
                            if ($gks_slider->options[GKSOption::kShowDetails] && !empty($details)) {
                                echo '<div class="gks-slider-desc">' . wp_kses($details, GKS_ALLOWED_HTML) . '</div>';
                            }
                ...
                <?php
                    ...
                    $showTextButton = !empty($gks_slider->options[GKSOption::kShowTextButton]) && $gks_slider->options[GKSOption::kShowTextButton];
                    $textButtonTitle = !empty($gks_slider->options[GKSOption::kTextButtonTitle]) ? $gks_slider->options[GKSOption::kTextButtonTitle] : "READ MORE";
                    ...
                    $showInfoBox = $showTitle || $showDetails || $showTextButton;
                    if ($showInfoBox) {
                        $infoBox =  '<div class="gks-slider-onslide-details gks-slider-slide-caption">';
                        $infoBox .= $showTitle ? '<div class="gks-slider-title">' . wp_kses($title, GKS_ALLOWED_HTML) . '</div>' : "";
                        $infoBox .= $showDetails ? '<div class="gks-slider-details">' . wp_kses($details, GKS_ALLOWED_HTML) . '</div>' : "";
                        $infoBox .= $showTextButton ? '<a class="gks-slide-txt-btn-' . esc_attr($textButtonDisplyStyle) . ' gks-slide-txt-btn gks-slide-txt-btn-primary" href="'. esc_url($url) .'" >' . wp_kses($textButtonTitle, GKS_ALLOWED_HTML) . '</a>' : "";
                        $infoBox .= '</div>';
                    }
                    ...
                    <div id="<?php echo esc_attr($slideId++); ?>" class="gks-slider-cell <?php echo esc_attr($pictureHoverEffect); ?> <?php echo esc_attr($cssselector); ?>" data-type="<?php echo esc_attr($coverType); ?>" data-index="<?php echo esc_attr($coverType != GKSAttachmentType::HTML ? $slideIndex++ : '') ?>" data-src="<?php echo esc_url(isset($metaOriginal['embed_src']) ? $metaOriginal['embed_src'] : ''); ?>">
                        <div class="gks-slider-cell-content gks-slider-cell-height" <?php echo 'data-url="'. esc_url($url) .'"'; ?>>
                          <?php if($coverType != GKSAttachmentType::HTML): ?>

                            <?php if ($showInfoBox && $infoDisplyStyle == GKSInfoDisplayStyle::Before) { echo $infoBox; } ?>
                            <div class="gks-img-container">
                              <div class="gks-slider-cell-content-image gks-tile-img" style="background-image: url(<?php echo esc_url($meta['src']); ?>);">
                            ...
                            <?php
                              if ($coverType == GKSAttachmentType::PICTURE) {
                                if ($showButton) {
                                  echo "<a href='" . esc_url($url) . "' class='gks-slider-button' ".($blank ? 'target="_blank"' : '')."><div class='gks-slider-button-icon'><i class='fa gks-fa fa-" . esc_attr($faButtonIcon) . "'></i></div></a>";
                                }
                              } else {
                                if (!empty($playIconHtml)) {
                                  echo $playIconHtml;
                            ...
                            <?php if ($showInfoBox && $infoDisplyStyle == GKSInfoDisplayStyle::After) { echo $infoBox; } //Already escaped html ?>
                            ...
                            <?php
                                ...
                                if ($infoDisplyStyle != GKSInfoDisplayStyle::Before && $infoDisplyStyle != GKSInfoDisplayStyle::After) {
                                  if ($infoOnHover) {
                                      ...
                                      if ($showInfoBox && !$isInfoOut) { $onSlide .= $infoBox; }
                                  } else {
                                      if ($showInfoBox && !$isInfoOut) { $onSlide .= $infoBox; }
                                      if ($showOverlay) { $onSlide .=  '<div class="gks-slider-overlay"></div>'; }
                                  }
                                ...
                                $onSlide .= '</div>';
                                echo $onSlide; //Already escaped html
                            ?>
                          <?php else: ?>
                            <?php echo wp_kses($coverHtml, GKS_ALLOWED_HTML); ?>
                          <?php endif; ?>
...
<script>
    var GKS_DATASOURCE_<?php echo esc_attr($gks_slider->id); ?> = [
      <?php foreach($lgDTS as $source): ?>
        <?php echo $source; //Already escaped html ?>,
      <?php endforeach; ?>
    ];

    var GKS_LG_OPTIONS_<?php echo esc_attr($gks_slider->id); ?> = <?php echo gks_inflatePopupOptions($gks_slider->id, $gks_slider->options); //Already escaped html ?>;

</script>

<?php foreach($lgVideosDTS as $sourceHTML): ?>
  <?php echo $sourceHTML; //Already escaped html ?>
<?php endforeach; ?>
...
<script>
    jQuery(document).ready(function(){
        ...
        var settings = {
            ...
            autoplayTimeout: <?php echo esc_attr(!empty($gks_slider->options[GKSOption::kSliderAutoplaySpeed]) ? $gks_slider->options[GKSOption::kSliderAutoplaySpeed]*1000 : 5000) ?>,
        ...
        <?php if ($isCarousel) { ?>
            settings.responsive = {
                0 : {
                    items: <?php echo esc_attr(!empty($gks_slider->options[GKSOption::kMobileCols]) ? $gks_slider->options[GKSOption::kMobileCols] : 1); ?>
                },
                <?php echo $mmxw+1; ?> : {
                    items: <?php echo esc_attr(!empty($gks_slider->options[GKSOption::kTabletCols]) ? $gks_slider->options[GKSOption::kTabletCols] : 2); ?>
                },
                <?php echo $dmnw; ?> : {
                    items: <?php echo esc_attr(!empty($gks_slider->options[GKSOption::kDesktopCols]) ? $gks_slider->options[GKSOption::kDesktopCols] : 3); ?>
                },
                <?php echo $dmxw+1; ?> : {
                    items: <?php echo esc_attr(!empty($gks_slider->options[GKSOption::kLgDesktopCols]) ? $gks_slider->options[GKSOption::kLgDesktopCols] : 4); ?>
                }
            };
            ...
            <?php if ($gks_slider->options[GKSOption::kSliderPaginationPosition] == GKSSliderPaginationPosition::BeforeSlider) { ?>
                jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .owl-carousel').prepend(jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .owl-dots'));
                jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .owl-dots').css('padding-bottom', '10px');
            <?php } ?>
                <?php
                if ($heightIsProportional) {
                $parts = explode('x', $gks_slider->options[GKSOption::kTileProportion]);
                ?>
                gks_AdjustSliderProportions(<?php echo esc_attr($gks_slider->id); ?>, <?php echo esc_attr($parts[0]/$parts[1]); ?>);
                <?php } ?>
            };
        ...
        var gksSlider = jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .owl-carousel').owlCarousel(settings);
        ...
        jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-ctrl-prev').click(function() {
            jQuery(this).closest('.gks-slider-layout').find('.owl-carousel').trigger('prev.owl.carousel', [1000]);
        });

        gksSlider.on('changed.owl.carousel', function(event) {
            jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-info-opened .gks-slider-info-toggle").click();
        });

        jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-ctrl-next').click(function() {
            jQuery(this).closest('.gks-slider-layout').find('.owl-carousel').trigger('next.owl.carousel', [1000]);
        });

        jQuery(window).resize(function(){
            gks_AdjustSlider(jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?>"));
        });

        gks_AdjustSlider(jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?>"));

        jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-info-toggle').click(function(){
            ...
            if (caption.hasClass('gks-info-opened')) {
                ...
                jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-ctrl').animate({opacity: 0}, 100);
            } else {
                ...
                jQuery('#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-ctrl').animate({opacity: 1}, 100);
            }
            return false;
        });
        ...
        jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-cell-content").on("click", function(evt){
          if (jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?> .gks-slider-onslide-details").has(evt.target).length) {
            return;
          }
        ...
        if (<?php echo $isAutoplayVideo && !$isCarousel ? "true" : "false"; ?>) {
          jQuery("#gks-slider-<?php echo esc_attr($gks_slider->id); ?> #0.gks-slider-cell").trigger("click");
        }
    });
