...
* Description: WordPress slider plugin! Slide your images, videos or any html content on your website with few clicks.
* Author: yooslider
...
//Load configs
    ...
    add_filter('http_request_args', 'gks_api_request_args',10,2);

    require_once(GKS_CLASSES_DIR_PATH . '/GKSCssBuilder.php');
...
//Register action hooks
    ...
    if (GKS_PKG_TYPE == GKS_PKG_TYPE_PREMIUM) {
      add_action('wp_ajax_gks_validate_license', 'wp_ajax_gks_validate_license');
...
//Registered installation/uninstallation/activation/deactivation hooks
    ...
    function gks_api_request_args($request_args, $url) {
          if (strpos($url, GKS_API_DOMAIN) !== false) {
    ...
    function gks_plugin_update_message( $data, $response ) {
        ...
        if ($info && !empty($info['update_message']) && ($info['status'] == GKSLicenseManager::STATUS_EXPIRING || $info['status'] == GKSLicenseManager::STATUS_EXPIRED || $info['status'] == GKSLicenseManager::STATUS_INVALID)) {
            printf( wp_kses($info['update_message'], GKS_ALLOWED_HTML));
        }
    }
...
//Shortcode Hanlders
    function gks_render_shortcode($attributes)
    {
        ...
        $sliderId = $attributes['id'];

        //Show template if it's required
        if (isset($_GET['template']) && (int)$_GET['template']) {
          $sliderId = (int)$_GET['template'];
        }
    ...
    function gks_get_all_sliders() {
      return GKSHelper::getSliderIdentifiers();
    }

//Internal functionality
    function gks_setup_admin_menu_buttons()
    {
        add_menu_page(GKS_PLUGIN_NAME, GKS_PLUGIN_NAME, 'manage_options', GKS_PLUGIN_SLAG, "gks_admin_sliders_page", 'dashicons-slides');
        add_submenu_page(GKS_PLUGIN_SLAG, GKS_SUBMENU_SLIDER_TITLE, GKS_SUBMENU_SLIDER_TITLE, 'manage_options', GKS_PLUGIN_SLAG, 'gks_admin_sliders_page');

        add_submenu_page(GKS_PLUGIN_SLAG, GKS_SUBMENU_TEMPLATES_SLUG, GKS_SUBMENU_TEMPLATES_TITLE, 'manage_options', GKS_SUBMENU_TEMPLATES_SLUG, 'gks_admin_templates_page');

        if (GKS_PKG_TYPE == GKS_PKG_TYPE_PREMIUM) {
          add_submenu_page(GKS_PLUGIN_SLAG, GKS_SUBMENU_LICENSE_SLUG, GKS_SUBMENU_LICENSE_TITLE, 'manage_options', GKS_SUBMENU_LICENSE_SLUG, 'gks_admin_license_page');
        }
    }
    ...
    function gks_admin_templates_page(){
        include_once(GKS_ADMIN_VIEWS_DIR_PATH."/gks-admin-templates.php");
    }
    ...
    function gks_include_inline_scripts()
    {
        ...
        <script type="text/javascript">
            ...
            gks_shortcodes.push({
                "id": "<?php echo esc_attr($gks_shortcode->id) ?>",
                "title": "<?php echo esc_attr($gks_shortcode->title) ?>",
                "shortcode": "<?php echo esc_attr($gks_shortcode->shortcode) ?>"
            });
            <?php endforeach; ?>
    ...
    function gks_enqueue_front_csss()
    {
        ...
        foreach ($sliders as $sliderId) {
          ...
          $builder = new GKSCssBuilder();
          $css = $builder->build($gks_slider, true);
