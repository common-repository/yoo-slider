...
class GKSLicenseManager {
    ...
    public function canUse($key = '')
    {
        $licenseKey = !empty($key) ? $key : $this->getKey();
    ...
    {
        if (is_null(GKSLicenseManager::$banners)) {
          ...
          if ($lastLoadedAt == 0 || time() - $lastLoadedAt > 1 * 24 * 60 * 60) {
            ...
            GKSLicenseManager::$banners = $cache;
        }
        if (!empty(GKSLicenseManager::$banners[$zone])) {
            return GKSLicenseManager::$banners[$zone];
        }
        return array();
    }
